...
instance (a b : ℕ) : ((a * b : ℕ) : ℂ) = (a : ℂ) * (b : ℂ) := by exact Nat.cast_mul a b 

instance (a b : ℕ) : ((a + b : ℕ) : ℂ) = (a : ℂ) + (b : ℂ) := by exact Nat.cast_add a b 

-- instance (a b : ℕ) : ((a - b : ℕ) : ℂ) = (a : ℂ) - (b : ℂ) := by exact Nat.cast_sub a b

instance (a b c : ℕ) (h : c ≤ a): a = b + c ↔ a - c = b := by exact Iff.symm (Nat.sub_eq_iff_eq_add h)

instance (a b c : ℂ) : (a - b) / c = a / c - b / c := by exact sub_div a b c

instance (a b c : ℂ) (h : c ≠ 0) (h₂ : a ≠ 0) : a / (a * c) = 1 / c := by exact div_mul_right c h₂

instance (a : ℕ) : (a : ℂ) = ((a : ℤ) : ℂ) := by simp only [cast_ofNat]

instance (a b q : ℕ) (h : a ≡ b [MOD q]) : (a : ZMod q) = (b : ZMod q) := by 
  exact Iff.mpr (ZMod.eq_iff_modEq_nat q) h

/- You're gonna have some headache reviewing the below proof -/
theorem AddChar_rationalFunc_deriv_eq_exp (y : ℤ) (hα : 0 < α) :
  ...
  /-
  # Ask Kevin
  nth_rw 1 [mul_comm 2 α]
  nth_rw 2 [pow_mul]
  -/
  
  
  /-
  have ha_lt : ((a : ZMod (p ^ (2 * α))) * rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α)) * z * (p ^ α : ℕ)).val < (p ^ (2 * α))
  { exact ZMod.val_lt (↑a * rationalFunc_deriv g₁ g₀ y (p ^ (2 * α)) * ↑z * ↑(p ^ α))
    sorry
  }
  -/
  ...
  rw [ZMod.val_cast_of_lt (pPow_lt_pTwoPow hp hα)]
  rw [mul_div_assoc]
  have ha_lt : ((a : ZMod (p ^ (2 * α))) * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α))) * z).val * (p ^ α : ℕ) % (p ^ (2 * α)) < (p ^ (2 * α))
  { 
    sorry
  }
  have ha_mod_modEq :=  Nat.mod_modEq (((a : ZMod (p ^ (2 * α))) * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α))) * z).val * (p ^ α : ℕ))

  have : ((a : ZMod (p ^ (2 * α))) * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α))) * z).val * (p ^ α : ℕ) % (p ^ (2 * α)) ≡ ((a : ZMod (p ^ (2 * α))) * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α))) * z).val * (p ^ α : ℕ) [MOD (p ^ (2 * α))] := by 
    exact Nat.mod_modEq (ZMod.val (↑a * rationalFunc_deriv g₁ g₀ y (p ^ (2 * α)) * ↑z) * p ^ α) (p ^ (2 * α))
  -/ 
  
  -- simp only [Nat.cast_pow, ZMod.nat_cast_val]


  -- Nat.mod_add_div' (m k : ℕ) : m % k + m / k * k = m

  /-
  have val_mul_coe_split' : ((a : ZMod (p ^ (2 * α))) * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α)) * z * (p ^ α : ℕ))).val ≡ (a : ZMod (p ^ (2 * α))).val * (rationalFunc_deriv (g₁) (g₀) (y) (p^(2*α)) * z * (p ^ α : ℕ)).val [ZMOD (p ^ (2 * α))]
  { rw [ZMod.val_mul]
    rw [ofNat_emod, Nat.cast_mul]
    exact Int.mod_modEq (((a : ZMod (p ^ (2 * α))).val : ℤ) * ((rationalFunc_deriv g₁ g₀ y (p ^ (2 * α)) * (z : ZMod (p ^ (2 * α))) * (p ^ α : ZMod (p ^ (2 * α)))).val : ℤ)) (p ^ (2 * α) : ℤ)
  }
  -/ 
